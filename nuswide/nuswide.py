...
def get_fn_map(path="/data1/sunyuchen/datasets/NUSWIDE/nuswide/Flickr/"):
    fn_map = {}
    for fn in glob.glob(path + r"*/*.jpg"):
        tmp = fn.split('_')[-1]
        fn_map[tmp] = fn
    return fn_map
...
def read_info(root, set):
    ...
    hash2ids = {}
    if set == "trainval":
        path = root + "/ImageList" + "/TrainImagelist.txt"
    elif set == "test":
        path = root + "/ImageList" + "/TestImagelist.txt"
    ...
    with open(file, 'r') as f:
        ...
        for row in reader:
            if header and rownum == 0:
                ...
                labels = torch.from_numpy((labels + 1) / 2)
                if labels.sum() > 0:
                    name2 = fn_map[imagelist[name] + ".jpg"]
                    item = (name2, labels)
                    images.append(item)
            rownum += 1
    return images
...
class NUSWIDEClassification(data.Dataset):
    def __init__(self, root, set, transform=None, target_transform=None):
        ...
        path_csv = self.root + '/classification_labels'
        # define filename of csv file
        file_csv = path_csv + '/classification_' + set + '.csv'
